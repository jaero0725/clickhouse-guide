...
## 5.10 시각적 이해 — 잘못된 파티셔닝이 파트 폭증을 일으키는 과정
...
PARTITION BY toDate(event_time);  -- ← 날짜 단위 파티션
```

3년치 데이터 = `365 × 3 = 1,095개` 파티션 생성

### INSERT 할 때 일어나는 일

```
INSERT 1만 건 (여러 날짜 섞임)
  ├── 2024-01-01 데이터 50건  →  파티션 2024-01-01 에 파트 생성
  ├── 2024-01-02 데이터 80건  →  파티션 2024-01-02 에 파트 생성
  ├── 2024-01-03 데이터 30건  →  파티션 2024-01-03 에 파트 생성
  └── ...
  INSERT 1번에 날짜 30개 섞이면 → 파트 30개 동시 생성
```

### 머지 제약이 왜 문제인가

```
파티션 2024-01-01 안의 파트들  ← 얘네끼리만 머지 가능
파티션 2024-01-02 안의 파트들  ← 얘네끼리만 머지 가능
파티션 2024-01-03 안의 파트들  ← 얘네끼리만 머지 가능
...
...
```
1,095개 파티션 × 파티션당 파트 수십 개
= 전체 파트 수만 개

→ "Too many parts" 에러 (기본 임계값 3,000개 초과)
→ INSERT 자체가 거부됨
```

### 파티셔닝 유무 실측 비교 (동일 데이터, 동일 쿼리)

```sql
SELECT MAX(price) FROM table WHERE town = 'LONDON';
-- town은 파티션 키가 아님 → 파티션 프루닝 미작동
```

| 지표 | 파티셔닝 없음 | 날짜 단위 파티셔닝 |
|------|-------------|-----------------|
| 활성 파트 수 | **1개** (완전 머지됨) | **436개** |
| 스캔 granule 수 | 241개 | 671개 |
| 처리 행 수 | 197만 | 548만 |
| 소요 시간 | **12ms** | **90ms** (7.5배 느림) |
| 메모리 | 62MB | 163MB (2.6배) |

파티셔닝이 오히려 느린 이유: 436개 파트를 각각 열어 인덱스 검사하는 오버헤드
...
-- 3년치 = 36개 파티션, 관리 가능한 수준

-- 파티셔닝의 실제 목적: "데이터 관리"
-- 오래된 월 데이터 즉시 삭제 (메타데이터 조작, 수억 건도 즉시)
ALTER TABLE user_events DROP PARTITION '2022-01-01';
...
```
✅ TTL로 오래된 데이터 자동 삭제
✅ 오래된 파티션을 S3/HDD로 이동 (hot/cold 분리)
✅ 파티션 단위 빠른 DROP / DETACH / MOVE
❌ 쿼리 성능 최적화 → 그건 PRIMARY KEY 가 담당
```
