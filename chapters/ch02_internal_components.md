...
## 2.12 시각적 실행 흐름 — 쿼리 하나를 끝까지 따라가기

아래 예시 쿼리를 기준으로 Parser부터 클라이언트 응답까지 전 단계를 시각적으로 따라간다.
...
### Step 1 — Parser: SQL 문자열 → AST

```
입력: "SELECT event_type, max(amount) FROM user_events WHERE user_id = 123 GROUP BY event_type"

                    SelectQuery
                   /     |      \
              columns    from   where         groupBy
             /      \     |       |              |
       Column     Function  Table   Equals     Column
     (event_type) (max)  (user_events)  / \   (event_type)
                    |               Column  123
                  Column          (user_id)
                 (amount)
```

### Step 2 — Interpreter: AST → QueryPipeline

```
InterpreterSelectQuery가 분석:
  1. user_events 테이블에서
  2. user_id = 123 으로 필터하고
  3. event_type, amount 컬럼만 읽고
  4. GROUP BY event_type + max(amount) 집계

→ Processor들의 DAG(실행 그래프) 생성
```

### Step 3 — QueryPipeline: Processor DAG (병렬 실행 구조)

```
디스크
  │
  │  파트 A (level 1)    파트 B (level 1)    파트 C (level 2)
  │       │                   │                   │
  ▼       ▼                   ▼                   ▼
┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│ Source       │     │ Source       │     │ Source       │
│ Processor A  │     │ Processor B  │     │ Processor C  │
│ (파트1,2 읽기)│     │ (파트3,4 읽기)│     │ (파트5,6 읽기)│
└──────┬───────┘     └──────┬───────┘     └──────┬───────┘
       │                    │                    │
       ▼                    ▼                    ▼
┌──────────────┐     ┌──────────────┐     ┌──────────────┐
...
Source ~ Filter: 파트별로 병렬 실행
Aggregation:     결과를 하나로 합쳐서 처리
```

### Step 4 — Source Processor: 디스크에서 실제로 읽기

```
파트 디렉터리 구조:
  user_events/all_0_5_1/
    ├── primary.idx      ← 항상 메모리에 올라와 있음
    ├── user_id.bin      ← 압축된 user_id 컬럼 데이터
    ├── event_type.bin
    ├── amount.bin
    ├── user_id.mrk      ← 오프셋 마크 (granule → 파일 위치)
    ├── event_type.mrk
    └── amount.mrk

읽기 과정 (Data Pruning):

  ① primary.idx 검사 (이미 메모리에 있음)
     ┌──────────────────────────────────────────┐
     │ granule 0: user_id 시작값 =      1       │
     │ granule 1: user_id 시작값 =  8,193       │
     │ granule 2: user_id 시작값 = 16,385       │ ← 123은 granule 0에 있음
     │ ...                                      │
     └──────────────────────────────────────────┘
     → 이진 탐색으로 user_id=123이 있을 granule 특정
     → 나머지 granule은 디스크에서 읽지도 않음 (Pruning)

  ② .mrk 파일로 해당 granule의 파일 오프셋 계산

  ③ user_id.bin, event_type.bin, amount.bin 에서
     해당 구간만 읽고 압축 해제

  ④ Block 생성 (최대 8,192행)
```

### Step 5 — Block이 파이프라인을 흐르는 모습

```
Source Processor 출력 Block (8,192행 묶음):
┌────────────────────────────────────────────┐
│  user_id │  event_type  │  amount          │
├──────────┼──────────────┼──────────────────┤
│   101    │   "click"    │    500           │
│   123    │  "purchase"  │  15000  ← 필요   │
│   456    │   "view"     │      0           │
│   123    │   "click"    │    200  ← 필요   │
│   789    │  "purchase"  │   8000           │
│   ...    │     ...      │    ...           │
└────────────────────────────────────────────┘
                │
                ▼ Filter Processor (WHERE user_id = 123)
                  IColumn::filter(mask=[0,1,0,1,0,...]) 호출
                  원본 Block은 그대로 → 새 Block 생성 (immutable)
                │
                ▼
┌────────────────────────────────────────────┐
│  user_id │  event_type  │  amount          │
...
└────────────────────────────────────────────┘
                │
                ▼ Aggregation Processor (GROUP BY event_type, max(amount))
                  내부 해시테이블:
                  { "purchase" → max_state{15000} }
                  { "click"   → max_state{200}   }
                │
                ▼
┌─────────────────────────────┐
│  event_type  │  max(amount) │
├──────────────┼──────────────┤
│  "purchase"  │    15000     │
│  "click"     │      200     │
└─────────────────────────────┘
                │
                ▼ Output Processor → JSON 직렬화 → 클라이언트
```

### Step 6 — Vectorized 실행 vs row-at-a-time

```
일반 DB (row-at-a-time):
  행 1: max(15000) 호출  → 함수 호출 1번
  행 2: max(200)   호출  → 함수 호출 1번
  행 3: max(8000)  호출  → 함수 호출 1번
  N행 → N번 함수 호출

ClickHouse (vectorized):
  amount 배열: [15000, 200, 8000, 500, ...]
               ████████████████████████
               CPU SIMD 명령어로 4~8개씩 한 번에 처리
  N행 → N/8번만 함수 호출 (+ 캐시 히트율 극대화)
```

### Step 7 — Short-circuit 평가가 없는 이유

```
일반 DB:
  WHERE f(x) AND g(y)
  row 1: f(x)=false → g(y) 건너뜀 (단락 평가)
  row 2: f(x)=true  → g(y) 계산

ClickHouse:
  [f(x1), f(x2), f(x3), ...]  → 배열 전체 한 번에 계산
  [g(y1), g(y2), g(y3), ...]  → 배열 전체 한 번에 계산
  → AND 연산으로 합침
  벡터화 실행 = 조건 분기 없음 → f(x)=false여도 g(y)가 계산됨

g(y)가 매우 비싼 연산이라면?
  → 서브쿼리로 f(x) 필터링 먼저 수행 후 g(y) 계산
```
...
```
              ...
              └───────┬────────┘
                      │
              ┌───────▼────────┐
              │  Interpreter   │  AST → QueryPipeline
              └───────┬────────┘
                      │
              ┌───────▼────────┐
              │PipelineExecutor│  Pipeline 실행 스케줄링
              └───────┬────────┘
          ┌───────────┼───────────┐  ← 파트별 병렬 실행
          │           │           │
    ┌─────▼─────┐ ┌───▼─────┐ ┌──▼──────┐
    │Source A   │ │Source B │ │Source C │  파트 읽기 + Pruning
    │Filter A   │ │Filter B │ │Filter C │  WHERE 처리
    └─────┬─────┘ └───┬─────┘ └──┬──────┘
          └───────────┼───────────┘
                ┌─────▼──────┐
                │Aggregation │  GROUP BY / 집계 함수
                └─────┬──────┘
                ┌─────▼──────┐
                │  Output    │  포맷 직렬화
                └─────┬──────┘
...
데이터 단위: Block (~8,192행 묶음)
각 Processor는 Block을 받아 새 Block을 내보냄 (immutable)
```
